...
Subagent (general-purpose):
  ...
  prompt: |
    ...
    Binding constraints from the plan/spec that bind this task:
    [BINDING_CONSTRAINTS]
    ...
    **Diff file:** [DIFF_FILE]
    ...
    `git diff [BASE_SHA]..[HEAD_SHA]`. Do not crawl the broader codebase. Inspect
    code outside the diff only to evaluate a concrete risk you can name — one
    focused check per named risk, and name both the risk and what you checked in
    your report. Cross-cutting changes are legitimate named risks: if the diff
...
- `[DIFF_FILE]` — REQUIRED: the path the controller wrote the review package to
  (`review-package BASE HEAD` prints the unique path it wrote; the package never
